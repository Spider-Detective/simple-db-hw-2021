...
import static simpledb.index.BTreePageId.LEAF;
...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeLeafPage newPage = (BTreeLeafPage) getEmptyPage(tid, dirtypages, page.getId().pgcateg());
		Iterator<Tuple> iterator = page.reverseIterator();
		int half = page.getNumTuples() / 2;
		while (iterator.hasNext() && half > 0) {  // move half of the tuples to the new page
			Tuple next = iterator.next();
			// must delete first, because insert will set a new record id to the tuple
			page.deleteTuple(next);
			newPage.insertTuple(next);
			half--;
		}

		// set center to a new entry in parent
		Tuple center = iterator.next();
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parentPage.insertEntry(new BTreeEntry(center.getField(keyField), page.getId(), newPage.getId()));

		// update sibling pointers
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage origRightPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			origRightPage.setLeftSiblingId(newPage.getId());
			dirtypages.put(origRightPage.getId(), origRightPage);
		}
		...
		return field.compare(Op.LESS_THAN_OR_EQ, center.getField(keyField)) ? page : newPage;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// should be inserted.
		BTreeInternalPage newPage = (BTreeInternalPage) getEmptyPage(tid, dirtypages, page.getId().pgcateg());
		...
		// move center to a new entry in parent, and delete it from the children
		BTreeEntry center = iterator.next();
		page.deleteKeyAndRightChild(center);
		center.setLeftChild(page.getId());
		center.setRightChild(newPage.getId());
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parentPage.insertEntry(center);

		// update parent pointers
		page.setParentId(parentPage.getId());
		newPage.setParentId(parentPage.getId());
		// newPage is an internal page, so need to update its children's parent pointers to it
		...
		return field.compare(Op.LESS_THAN_OR_EQ, center.getKey()) ? page : newPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int expectedSize = (page.getNumTuples() + sibling.getNumTuples()) / 2;
		int moveSize = expectedSize - page.getNumTuples();;
		if (moveSize <= 0) {
			return;
		}
		...
		Tuple next = null;
		while (iterator.hasNext() && moveSize > 0) {
			next = iterator.next();
		...
		// parent key needs to be the first tuple of the right node
		// if move tuples from left to right, parent key will be `next`
		// if move tuples from right to left, parent key will be `iterator.next()`
		Tuple newParentKey = isRightSibling ? iterator.next() : next;
		entry.setKey(newParentKey.getField(keyField));
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int expectedSize = (page.getNumEntries() + leftSibling.getNumEntries()) / 2;
		int moveSize = expectedSize - page.getNumEntries();
		if (moveSize <= 0) {
			return;
		}

		// move the parent entry to right child, need to keep the left and right child
		BTreeEntry midEntry = new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(midEntry);
		moveSize--;

		Iterator<BTreeEntry> iterator = leftSibling.reverseIterator();
		// move entry from right to left, need to delete right child
		// otherwise the leftSibling can still get to the right child by accessing children[]
		while (iterator.hasNext() && moveSize > 0) {
			BTreeEntry next = iterator.next();
		...
		BTreeEntry newParentEntry = iterator.next();
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int expectedSize = (page.getNumEntries() + rightSibling.getNumEntries()) / 2;
		int moveSize = expectedSize - page.getNumEntries();
		if (moveSize <= 0) {
			return;
		}

		// move the parent entry to left child, need to keep the left and right child
		BTreeEntry midEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
		page.insertEntry(midEntry);
		moveSize--;

		Iterator<BTreeEntry> iterator = rightSibling.iterator();
		// move entry from right to left, need to delete left child
		// otherwise the rightSibling can still get to the left child by accessing children[]
		while (iterator.hasNext() && moveSize > 0) {
			BTreeEntry next = iterator.next();
